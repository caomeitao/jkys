...
async function handleAnalyze() {
    ...
    try {
        // 调用阿里云百炼智能体
        const response = await callBailianAgent(foodText);
...
// 调用阿里云百炼智能体
async function callBailianAgent(foodText) {
    try {
        const url = `https://dashscope.aliyuncs.com/api/v1/apps/${CONFIG.APP_ID}/completion`;
        ...
        } else if (data.output && data.output.finish_reason) {
            throw new Error('智能体返回了空响应');
        } else {
            throw new Error('返回数据格式错误: ' + JSON.stringify(data));
        }

    } catch (error) {
        console.error('调用智能体出错:', error);
        throw error;
    }
}
